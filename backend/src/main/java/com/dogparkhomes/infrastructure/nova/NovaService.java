    ...
    public SearchFiltersDto parseUserQuery(String query) {
        ...
        try {
            ...
            String prompt = """
                    ...
                    Price range: If the user specifies a price or price range, set "price_range" to a string using NUMERIC VALUES ONLY. Convert shorthand to digits: "0.5m" or "0.5M" -> 500000, "1m" or "1M" -> 1000000, "900k" or "0.9m" -> 900000. Format rules: (1) Single number or "around X" -> use ±20%% of that value: min = 0.8*X, max = 1.2*X (e.g. 500000 -> "400000:600000"). (2) "At most X" / "under X" / "max X" / "no more than X" -> "*:X" (e.g. at most 900k -> "*:900000"). (3) "At least X" / "over X" / "min X" -> "X:*" (e.g. at least 800k -> "800000:*"). (4) Explicit range "A to B" or "between A and B" -> "minPrice:maxPrice" (e.g. "800000:1200000"). Use "*" only for unbounded min or max. If no price is mentioned, set "price_range" to null.
                    User query: %s
                    """.formatted(query);
